...
layui.use(["table","form","laytpl","layer","disaster"], function(){
    ...
    table.render({
        ...
        ,where:{
            currentFlow: 4
            ,isOption: 0
            ,areaId: employee.organizationType == 0 ? employee.areaId : "当前用户所在机构不是发布中心"
        } // 查询流程中预警编辑提交信息
        ,page:true
        ,even: true
        ,height: 'full-125'
        ,limits:[10,20,50,100]
        ,text: {
            none : employee.organizationType == 0 ? "暂无数据" : "<span class='ewip-warn-table-msg'>当前用户所在机构不是发布中心，不具备发布共功能</span>"
        }
        ,cols: [[
            {type: 'checkbox'}
